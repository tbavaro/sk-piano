...
static void setPinDirection(int number, SlowPin::Mode mode) {
  const char* direction = (mode == SlowPin::INPUT ? "in" : "out");
  writeFile(getGpioPath(number) + "/direction", direction);
}

static void setPinDirectionMaybeDeferred(int number, SlowPin::Mode mode) {
...
SlowPin::SlowPin(const string& name, int number) : Pin(number), name(name) {
...
void SlowPin::setMode(Mode mode) {
